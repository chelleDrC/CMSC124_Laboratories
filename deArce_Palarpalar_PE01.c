...
int isValidPostfix(const char *str);
...
int isValidPostfix(const char *str)
{
    ...
    while (*str)
    {
        ...
        else if (*str == '+' || *str == '-' || *str == '*' || *str == '/' || *str == '%')
        {
            if (operandCount < 2)
            {
                return 0; // Not enough operands for an operator
            }
            operandCount--; // Reduce because two operands are combined into one
            str++;
        }
        else
        {
    ...
    return operandCount == 1; // Should end with exactly one result
}